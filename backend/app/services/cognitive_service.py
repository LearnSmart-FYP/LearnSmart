...
async def evaluate_script_cognitive_profile(script_id: str):
    """
    Background task to evaluate all knowledge points in a script if not already evaluated.
    """
    logger.info(f"Starting background cognitive profile generation for script: {script_id}")
    try:
        if not postgres_db.pool:
            logger.warning("Database pool is not connected, cannot run background task")
            return
        ...
        async with pool.acquire() as conn:
            script = await conn.fetchrow("SELECT id, title, outline_json FROM scripts WHERE id = $1", script_id)
            if not script:
                logger.warning(f"Script {script_id} not found for cognitive profile processing")
                return
            ...
            if isinstance(outline, str):
                try:
                    ...
                    logger.warning(f"Script {script_id} has invalid JSON in outline_json")
                    return
            
            if not isinstance(outline, dict):
                logger.warning(f"Script {script_id} outline_json is not an object")
                return
        ...
        if not knowledge_points_to_process:
            logger.info(f"All knowledge points for script {script_id} have already been evaluated.")
            return
            
        logger.info(f"Found {len(knowledge_points_to_process)} unprofiled knowledge points for script {script_id}.")
        ...
        logger.info(f"Successfully processed {success_count}/{len(knowledge_points_to_process)} knowledge points for script {script_id}.")
    except Exception as e:
        logger.error(f"Error in background background cognitive profile generation: {e}")
